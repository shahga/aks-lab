# *******************************************************************************
# ! OBJECTIVE: Kuberentes Storage: Azure Standard Storage, Azure Premium Storage
#  Have an AKS cluster and create two initial StorageClasses:
#  default - Uses Azure Standard storage to create a Managed Disk. 
#            The reclaim policy indicates that the underlying Azure Disk is deleted 
#            when the pod that used it is deleted.
#  managed-premium - Uses Azure Premium storage to create Managed Disk. 
#                     The reclaim policy again indicates that the underlying Azure Disk is deleted
#                     when the pod that used it is deleted.
# *******************************************************************************


cd C:\aks-lab\m13-Storage
...
# ********************************************
# ! KUBERNETES STORAGE: Azure Premium Storage
# ********************************************

# Additional storage classes can be created with kubectl. 
# Lets create a managed premium storage class that will retain the persistent volume after pod deletion:
...
# The K8S API server can dynamically provision the underlying storage resource in Azure if there is no existing resource to fulfill the claim based on the defined StorageClass. 
...
kubectl describe pv pvc-43a7d048-8d68-44ef-89ba-646c33b095d4
...
az aks show -g gslab-rgp -n gslab-aks --query nodeResourceGroup

# Then we can create a storage account with that resource group:
az storage account create -n "gslabmodule8storage" -g "gslab-nodes-rgp" -l "eastus" --sku Standard_LRS

# Now we can create the storage class, 
# and a role and role binding which are needed to allow the storage controller to create a secret for the PVC to use for accessing the file share resource.
